...
describe('platform scanners', () => {
  ...
  it('scans GitHub Actions env keys only when they require external configuration', async () => {
    await expect(variables(scanGitHubActions, {
      '.github/workflows/deploy.yaml': 'name: deploy\non: push\nenv:\n  DEPLOY_TOKEN: ${{ secrets.DEPLOY_TOKEN }}\n  DATABASE_URL: ""\n  NODE_VERSION: "22"\n  NODE_ENV: production\njobs:\n  deploy:\n    runs-on: ubuntu-latest\n    steps:\n      - run: echo "$GITHUB_SHA $GITHUB_RUN_ATTEMPT $GITHUB_SERVER_URL $GITHUB_REPOSITORY_OWNER $GITHUB_ACTION_PATH $GITHUB_ACTION_REPOSITORY $GITHUB_WORKFLOW_REF $GITHUB_WORKFLOW_SHA $RUNNER_ARCH $RUNNER_TEMP $RUNNER_TOOL_CACHE" && pnpm deploy --token $DEPLOY_TOKEN --node $NODE_VERSION --env $NODE_ENV --database $DATABASE_URL\n',
    })).resolves.toEqual([
  ...
  it('ignores Jenkins environment variables defined by credentials or literals', async () => {
    await expect(variables(scanJenkins, {
  ...
  stages {
    stage('Deploy') {
      steps {
        sh 'deploy --token $DEPLOY_TOKEN --railway-env $RAILWAY_ENVIRONMENT --api-url $API_URL --database $DATABASE_URL'
      }
    }
  }
}
